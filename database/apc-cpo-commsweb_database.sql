...
--
-- Host: localhost
-- Generation Time: Mar 09, 2015 at 04:46 PM
...
`id` int(11) NOT NULL,
  ...
  `contactnum` varchar(15) COLLATE utf8_unicode_ci NOT NULL,
...
INSERT INTO `user` (`id`, `username`, `firstname`, `lastname`, `auth_key`, `password_hash`, `password_reset_token`, `email`, `contactnum`, `roles`, `company_id`, `status`, `created_at`, `updated_at`) VALUES
...
(9, 'joshrramos', 'Josh', 'Ramos', '954kHhtkPlKz45KxIlSWodr1DWY-bIRu', '$2y$13$jtimqQghNQk/ge3oslXPz.KhBnn9LLdp1UJhCpdCBAl1AcfBur46y', NULL, 'jrramos@student.apc.edu.ph', '', 10, 0, 10, 1425884101, 1425884101);

--
-- Triggers `user`
--
DELIMITER //
CREATE TRIGGER `insert_cpo` AFTER UPDATE ON `user`
 FOR EACH ROW begin
...
DELIMITER ;
DELIMITER //
CREATE TRIGGER `insert_iprof_student` AFTER INSERT ON `user`
 FOR EACH ROW begin
INSERT INTO iproffessor (user_id, username, firstname, lastname, email, contact_num, company_id)
    SELECT user.id, user.username, user.firstname, user.lastname, user.email, user.contactnum, user.company_id
		from user where user.id not in (select iproffessor.user_id from iproffessor) && roles = 15;
        
delete from iproffessor where user_id in (select user.id from user where user.roles != 15);
end
//
DELIMITER ;
...
ADD CONSTRAINT `iprofessor_ibfk_3` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`);
